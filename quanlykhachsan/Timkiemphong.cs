...
namespace quanlykhachsan
{
    ...
    {      
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string tuKhoa = txtKey.Text.Trim().ToLower();
            ...
            if (cbTK.Text == "Tình trạng phòng")
            {
                ...
                SELECT * 
                FROM tbl_phong 
                WHERE LOWER(LTRIM(RTRIM(ISNULL(TINHTRANG, '')))) 
                LIKE N'%{tuKhoa.Replace("'", "''")}%'");
            }
            else if (cbTK.Text == "Loại phòng")
            {
                dataGridView1.DataSource = xemdl($@"
                SELECT * 
                FROM tbl_phong 
                WHERE LOWER(LTRIM(RTRIM(ISNULL(LOAIPHONG, '')))) 
                LIKE N'%{tuKhoa.Replace("'", "''")}%'");
            }
        }
